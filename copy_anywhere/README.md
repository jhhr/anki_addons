...
Every picture below is generated by `copy_anywhere/test/test_readme_screenshots.py` from the
example definitions in `docs/examples/`. This section is a temporary index of them, so that
nothing generated is left unreferenced while the rest of this file is being rewritten.

### The dialogs

![The definitions list](docs/images/definitions-list.png)
The list of copy definitions: pick which ones to run now, or open one for editing.

![The stage editor](docs/images/stage-editor.png)
One definition: what triggers it, the stages it runs in order, what it exports, and the
preview beside them.

### Building one, step by step

![Step 1: find related notes](docs/images/walkthrough-1-find-related-notes.png)
Step 1 of `collect-meanings.json`: a note query names the notes it found `Related`.

![Step 2: an empty list of meanings](docs/images/walkthrough-2-an-empty-list-of-meanings.png)
Step 2: an empty list called `Meanings`, for the loop to fill.

![Step 3: for each related note](docs/images/walkthrough-3-for-each-related-note.png)
Step 3: a loop over `Related`, with a Store stage putting each note's meaning in the list.

![Step 4: join them into one line](docs/images/walkthrough-4-join-them-into-one-line.png)
Step 4: a reduce joins the list into one piece of text called `Summary`.

![Step 5: write them into this note](docs/images/walkthrough-5-write-them-into-this-note.png)
Step 5: an Edit Note stage writes `Summary` into the trigger note's own field.
...
![Note query](docs/images/stage-note_query.png)
`note_query` — an Anki search, with how many of the matches to take and in what order.

![Card query](docs/images/stage-card_query.png)
...
![List variable](docs/images/stage-list_variable.png)
`list_variable` — declares an empty list.

![Store](docs/images/stage-store.png)
`store` — appends one value to a declared list.

![For each note](docs/images/stage-for_each_note.png)
`for_each_note` — a child scope per note in a note list.

![For each card](docs/images/stage-for_each_card.png)
`for_each_card` — a child scope per card, binding the card's note too.

![Reduce](docs/images/stage-reduce.png)
`reduce` — folds a list into one result, or joins it.
...
The card actions editor: one panel per card type the note can have.

![The tag editor](docs/images/tag-editor.png)
Tags to add and to remove, on whichever note the stage edits.

![The exports panel](docs/images/exports-panel.png)
The exports panel: which top-level results a calling definition may bind.

### Writing a value

![A text value](docs/images/value-text.png)
Text mode: references are interpolated, and the right-click menu lists what is in scope.

![A code value](docs/images/value-code.png)
Code mode: the body of a function that returns the value.

![An unknown reference](docs/images/value-unknown-reference.png)
A reference that names nothing in scope is marked in red under the box.

![A migrated expression](docs/images/value-legacy.png)
A migrated expression still spells its references format 1's way; edit its text and it is
read as current syntax, where the unqualified name refers to nothing.

### What the editor warns about

![The impossible warning](docs/images/add-note-impossible.png)
A card action on the note being added cannot run: the note has no cards yet.

![Both warnings at once](docs/images/add-note-both.png)
The same definition also writing a file: that would outlive a cancelled add, so the
definition runs after the add instead, under its own undo entry.

![A refused migrated selection](docs/images/migrated-selection-refused.png)
A selection format 1 refused to make keeps refusing until you say otherwise here.
