...
class TestTags:
    ...
    def test_an_empty_add_tags_adds_nothing_and_does_not_mark_the_note_modified(
        ...
        assert split_tags("") == []
        modified, _, _ = run(note, add_tags="", remove_tags="")
        assert modified is False
        assert note.tags == []
...
class TestFiles:
    ...
    def test_an_empty_filename_aborts_the_definition(self, note, logger):
        with pytest.raises(CopyFailedException):
            run(note, field_to_file_defs=[d.field_to_file("", "{{Word}}")])
    ...
    def test_a_malformed_code_result_aborts_the_definition(self, note, code, message):
...
class TestEditedFlag:
    def test_an_untouched_card_gets_no_edited_attribute(self, note):
        ...
        _, _, cards = run(note, card_actions=[])
