...
def call(col, trigger, extra_state=None, logger=None, **definition_kwargs):
...
class TestGuards:
    def test_a_missing_select_card_by_returns_nothing(self, col, trigger, targets, logger):
        assert call(col, trigger, logger=logger, copy_from_cards_query="tag:pool",
                    select_card_by=None) == []
    ...
    def test_a_missing_select_card_count_defaults_to_one(self, col, trigger, targets, logger, count):
        notes = call(col, trigger, logger=logger, copy_from_cards_query="tag:pool",
    ...
    def test_a_query_that_interpolates_to_nothing_is_an_error(self, col, trigger, logger):
        assert call(col, trigger, logger=logger, copy_from_cards_query="{{Note}}") == []
    ...
    def test_zero_results_are_logged_as_an_error_when_the_flag_is_set(self, col, trigger, logger):
        assert call(col, trigger, logger=logger, copy_from_cards_query="tag:nothing",
...
class TestSelection:
    ...
    def test_select_card_by_none_does_not_de_duplicate_notes(self, col, trigger, logger):
    ...
    def test_least_reps_picks_the_least_reviewed_then_the_next_least(
        ...
        notes = call(col, trigger, logger=logger,
    ...
    def test_asking_for_more_than_there_are_returns_what_there_is(
        ...
        notes = call(col, trigger, logger=logger, copy_from_cards_query="Word:w1",
                     select_card_by="None", select_card_count="10")
        assert [note.id for note in notes] == [targets[0].id]

    def test_select_card_count_zero_returns_distinct_notes(self, col, trigger, targets, logger):
        notes = call(col, trigger, logger=logger, copy_from_cards_query="tag:pool",
                     select_card_count="0")
...
class TestSorting:
    def test_sort_by_field_is_numeric_and_descending(self, col, trigger, targets, logger):
        notes = call(col, trigger, logger=logger, copy_from_cards_query="tag:pool",
    ...
    def test_a_non_numeric_value_sorts_as_zero_and_keeps_its_place(self, col, trigger, logger):
        ...
        notes = call(col, trigger, logger=logger, copy_from_cards_query="tag:s",
                     select_card_count="0", sort_by_field="Freq")
        assert [note["Word"] for note in notes] == ["y", "x", "z"]

    @pytest.mark.parametrize("sort_by_field", [None, "-"])
    def test_no_sorting_when_the_field_is_unset(self, col, trigger, targets, logger, sort_by_field):
        notes = call(col, trigger, logger=logger, copy_from_cards_query="tag:pool",
                     select_card_count="0", sort_by_field=sort_by_field)
        assert sorted(note.id for note in notes) == sorted(note.id for note in targets)

    def test_a_sort_field_absent_from_the_note_type_sorts_as_zero(self, col, trigger, logger):
        ...
        notes = call(col, trigger, logger=logger, copy_from_cards_query="tag:mixed",
                     select_card_count="0", sort_by_field="Freq")
        assert len(notes) == 2
...
class TestQueryCache:
    def test_the_same_interpolated_query_searches_once(self, col, trigger, targets, logger):
    ...
    def test_different_interpolated_queries_search_separately(self, col, targets, logger):
        ...
        for note in (first, second):
            call(col, note, extra_state=extra_state, logger=logger,
                 copy_from_cards_query="tag:pool {{Word}}", select_card_count="0")
        assert count() == 2

    def test_a_fresh_extra_state_searches_again(self, col, trigger, targets, logger):
        ...
        count = real_anki.counting_wrapper(col, "find_cards")
        for _ in range(2):
            call(col, trigger, extra_state={}, logger=logger,
                 copy_from_cards_query="tag:pool", select_card_count="0")
        assert count() == 2
...
class TestSharedStateWouldChangeSelection:
    ...
    def _select(self, col, trigger, extra_state, logger, count):
        return call(col, trigger, extra_state=extra_state, logger=logger,
    ...
    def test_per_note_state_gives_both_trigger_notes_the_same_answer(
        self, col, pool, logger, count
    ):
        first = real_anki.add_note(col, VOCAB, {"Word": "t1"})
        second = real_anki.add_note(col, VOCAB, {"Word": "t2"})
        one = self._select(col, first, {}, logger, count)
        two = self._select(col, second, {}, logger, count)
        assert [note.id for note in one] == [note.id for note in two]

    @pytest.mark.parametrize("count", ["1", "2"])
    def test_shared_state_gives_the_same_answer_here_too(self, col, pool, logger, count):
        ...
        second = real_anki.add_note(col, VOCAB, {"Word": "t2"})
        shared = {}
        one = self._select(col, first, shared, logger, count)
        two = self._select(col, second, shared, logger, count)
        assert [note.id for note in one] == [note.id for note in two]

    def test_shared_state_searches_once_where_per_note_state_searches_twice(
        self, col, pool, logger
    ):
        first = real_anki.add_note(col, VOCAB, {"Word": "t1"})
        second = real_anki.add_note(col, VOCAB, {"Word": "t2"})
        count = real_anki.counting_wrapper(col, "find_cards")
        shared = {}
        self._select(col, first, shared, logger, "1")
        self._select(col, second, shared, logger, "1")
        assert count() == 1
