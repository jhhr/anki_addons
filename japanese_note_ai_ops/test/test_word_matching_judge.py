...
class JudgeWordArrayTests(unittest.TestCase):
    def test_no_request_when_there_is_nothing_to_judge(self):
        ...
        ask = Asker({"dontmatch": []})
        self.assertIsNone(judge.judge_word_array(arr, match_flags.JUDGE_NEW, ask))
    ...
    def test_picks_are_dontmatch_and_the_rest_match(self):
        ...
        self.assertEqual(judge.judge_word_array(arr, match_flags.JUDGE_NEW, ask), [])
    ...
    def test_rejudging_reports_the_unlinked_note_ids(self):
        ...
        unlinked = judge.judge_word_array(arr, match_flags.REJUDGE_MATCHED, ask)
